...
bool Map::ScriptCommand_TeleportTo(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (pSource->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        if (script.teleportTo.teleportOptions & TELE_TO_RELATIVE)
            ...
            player->TeleportTo(script.teleportTo.mapId, script.x, script.y, script.z, script.o, script.teleportTo.teleportOptions);
    }
    else
    {
        if (script.teleportTo.teleportOptions & TELE_TO_RELATIVE)
            pSource->NearTeleportTo(
                script.x + target->GetPositionX(),
                script.y + target->GetPositionY(),
                script.z + target->GetPositionZ(),
                script.o + target->GetOrientation(),
        ...
        else pSource->NearTeleportTo(script, script.teleportTo.teleportOptions);
    }
...
bool Map::ScriptCommand_SendTaxiNode(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    pPlayer->ActivateTaxiPathTo(nodes, nullptr, 0, true);
